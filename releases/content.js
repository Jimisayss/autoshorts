...
// --- AutoShorts: All chrome.storage operations are now proxied via background.js --- //

/**
 * Sends a message to the background script to update stats.
 * @param {number} durationMs
 */
function updateStats(durationMs) {
  if (!chrome?.runtime?.id) {
    console.warn("AutoShorts: chrome.runtime.id not available, not sending updateStats.");
    return;
  }
  const msg = { action: "updateStats", durationMs };
  console.log("AutoShorts: Sending message to background:", msg);
  chrome.runtime.sendMessage(msg, (response) => {
    console.log("AutoShorts: Received response from background:", response);
    if (chrome.runtime.lastError) {
      console.error("AutoShorts: Error in updateStats message:", chrome.runtime.lastError.message);
    }
  });
}

/**
 * Sends a message to the background script to increment manual skip count.
 */
function incrementManualSkip() {
  if (!chrome?.runtime?.id) {
    console.warn("AutoShorts: chrome.runtime.id not available, not sending incrementManualSkip.");
    return;
  }
  const msg = { action: "incrementManualSkip" };
  console.log("AutoShorts: Sending message to background:", msg);
  chrome.runtime.sendMessage(msg, (response) => {
    console.log("AutoShorts: Received response from background:", response);
    if (chrome.runtime.lastError) {
      console.error("AutoShorts: Error in incrementManualSkip message:", chrome.runtime.lastError.message);
    }
  });
}
...
let lastWheelTime = 0;
let setupTimer = null; // For robust video setup retries

window.addEventListener("wheel", () => {
  lastWheelTime = Date.now();
}, { passive: true });

// Cancel running timers on page unload
window.addEventListener("beforeunload", () => {
  if (setupTimer) {
    clearTimeout(setupTimer);
    setupTimer = null;
    console.log("AutoShorts: Cleared setup timer on beforeunload.");
  }
});
  ...
  if (shortId && shortId !== lastShortId) {
    ...
    function trySetupVideo() {
      ...
      if (video) {
        ...
        attempts++;
        cleanupVideoListeners(currentVideo, onVideoPlaying, onVideoTimeUpdate);
        setupTimer = setTimeout(trySetupVideo, 200);
      } else {
        cleanupVideoListeners(currentVideo, onVideoPlaying, onVideoTimeUpdate);
...
// --- Bonus: SPA navigation detection ---
setInterval(() => {
  if (location.pathname !== lastPathname) {
    console.log("AutoShorts: Detected SPA navigation from", lastPathname, "to", location.pathname);
